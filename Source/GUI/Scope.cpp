...
void Scope::calculateScopeBuffers(void)
{
...
void Scope::audioDeviceAboutToStart (AudioIODevice*)
{
	zeromem (circularBuffer, sizeof (float) * bufferSize);
}

void  Scope::audioDeviceStopped()
{
	zeromem (circularBuffer, sizeof (float) * bufferSize);
}

void Scope::render (const AudioSourceChannelInfo& info)
{
    ScopedLock sl(bufferAccess);
...
//				inMax = outAudio [i][j];	//max of all channels
//		}
...
void Scope::audioDeviceIOCallback (const float** inAudio,int numInChannels,float** outAudio,int numOutAudio,int numSamples)
{
	ScopedLock sl(bufferAccess);
...
void Scope::paint (Graphics& g)
{
	calculateScopeBuffers();

//	g.fillAll (Colours::navy.darker());
...
void Scope::addSample (const float sample)
{
