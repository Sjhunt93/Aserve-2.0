...
void AserveAudio::parseMessage(const String& message)
{
    ...
    if(string.startsWith("osc"))
    {
        ...
        if(inRange("Osc Number", oscillatorNumber, 0, 15) && inRange("Osc Freq", freq, 0.f, 20000.f) 
           && inRange("Osc Amp", amp, 0.f, 1.f) && inRange("Osc Wave", wave, 0, 4))
        {
            if(amp > 0.f)
            {
...
void AserveAudio::setLPF (const int cuttoff)
{
    filter[0].setCoefficients(IIRCoefficients::makeLowPass(sampleRate, cuttoff));
    filter[1].setCoefficients(IIRCoefficients::makeLowPass(sampleRate, cuttoff));
}
...
void AserveAudio::audioDeviceIOCallback (const float** inAudio,int numInChannels,float** outAudio,int numOutAudio,int numSamples)
{
    ...
    filter[0].processSamples(outAudio[0], numSamples);
    filter[1].processSamples(outAudio[1], numSamples);
//    for (int i = 0; i < numSamples; i++) {
//        for (int c = 0; c < numOutAudio; c++) {
//            *outAudio[c] = filter[c].processSingleSampleRaw(outAudio[c]);
//        }
//    }
//    
...
void AserveAudio::handleIncomingMidiMessage(MidiInput *source, const MidiMessage &message)
{
    ...
    printf("Midid from device %s\n", source->getName().toRawUTF8() );
}
